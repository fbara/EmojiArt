...
//  EmojiArt
//
//  Created by Frank Bara on 12/7/20.
...
struct PalletteChooser_Previews: PreviewProvider {
    static var previews: some View {
        PalletteChooser(document: EmojiArtDocument(), chosenPalette: Binding.constant(""))
    }
}
        ...
        VStack(spacing: 0) {
            ...
            Form {
                ...
                Section(header: Text("Remove Emoji")) {
                    Grid(chosenPalette.map { String($0) }, id: \.self) { emjoi in
                        Text(emjoi).font(Font.system(size: self.fontSize))
                            .onTapGesture {
                                self.chosenPalette = self.document.removeEmoji(emjoi, fromPalette: self.chosenPalette)
                            }
                    }
